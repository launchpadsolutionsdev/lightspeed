...
**A practical guide to embedding Lightspeed into the BUMP platform/dashboard — from someone who uses it every day.**

---

If you're reading this, it means we're getting serious — and I'm excited about that (Hi Matt! xD).
...
I've been logging into BUMP's dashboard almost every day since we became a partner. I manage draws, resend tickets, pull reports, handle customer issues, and coordinate with your client team on a weekly basis. So when I think about where Lightspeed fits, I'm not theorizing per say. I'm thinking about the actual workflow I go through every morning when I sit down at my desk (btw, Maddie and the team are great).

Here's what I'd recommend, broken into three phases. The idea is to start with something your team can ship quickly, prove adoption, and then go deeper over time based on what your clients actually use. It genuinely excited me to think about the possibilities.

---

## Phase 1: Single Sign-On and Side-by-Side Access

The fastest path to getting Lightspeed in front of BUMP's clients is to let them access it directly from the BUMP dashboard without needing a separate login. A user clicks something like "Open Lightspeed" from within BUMP, and they land inside and are already authenticated, already pointing at their organization.

No separate credentials. No second onboarding flow. Just a seamless handoff from BUMP's auth system to Lightspeed's.

This is the lowest-effort starting point, and it immediately lets your team gauge interest and adoption across your client base without committing to a deeper build. If clients are using it, you invest in Phase 2. If they're not, you learn that early and pick which pieces of the platform you want to build out in other ways. I feel like the response assistant needs to be available to everyone from day one, it's genuinely a GAME-CHANGER.
...
Right now, when a new organization starts using Lightspeed, they have to manually configure everything: their licence number, draw schedule, store locations, brand voice, terminology preferences, website, mission statement, and then build out their knowledge base on top of that. It takes time, and it's the single biggest barrier to adoption. If an operator doesn't put in the setup work, the AI output is generic — and generic output gets ignored. I was going to remedy this by (if we brought it to market ourselves) going through an onboarding session and helping make this process easy...

BUT! Here's the thing: **BUMP already has almost all of this data for every client.** You know their licence numbers, their draw configurations, their store details, their raffle types, their operating jurisdictions. That is TONS of onboarding work that could be eliminated overnight.

If BUMP pushes that data into Lightspeed when a client activates — through a webhook, a sync, or even a one-time data load — the operator gets a pre-configured, pre-branded AI assistant from day one instead of a blank slate. The Response Assistant already knows their draw schedule. The Draft Assistant already knows their brand voice (sort of, I have ideas for this too).
...
- **Own the knowledge base content strategy.** A generic knowledge base produces generic output (I would love to help develop this, I've lived through every possible customer complaint and written every piece of marketing material). Lightspeed's Compliance Assistant was built with deep Ontario AGCO knowledge, but BUMP operates across multiple provinces with different regulatory frameworks. Seeding jurisdiction-specific knowledge bases for Alberta, British Columbia, and other provinces would be a major differentiator that no other raffle platform can offer. Even if its just something used for guidance, people LOVE to use Chat-GPT and LLM's, conversational AI is not going anywhere.
