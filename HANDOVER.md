...
## What you're actually getting (feature inventory)

The README lists 5 tools. The real platform is quite a bit bigger than that. Here's what actually exists:

**AI tools:**
- **Response Assistant** — the core tool. Staff paste a customer inquiry, pick tone/length/format/language, and get an AI-drafted response. Under the hood it pulls from the org's knowledge base, applies response rules, injects Shopify context if connected, and matches the org's voice fingerprint.
- **Draft Assistant** — content generation for social posts, emails, ads, lottery announcements. Has its own voice profile separate from Response Assistant.
- **Ask Lightspeed** — general-purpose AI chat with conversation threading, web search, citations, and model selection. This is the one that grew the most toward the end.
- **Insights Engine** — upload Excel/CSV data, get automated analysis and visualizations.
- **Data Agent** — list cleaning and deduplication for customer/seller data.
- **Compliance Assistant** — multi-turn conversational AI grounded in Ontario AGCO lottery licensing policy. Uses 12 jurisdiction-specific KB files (`backend/data/ontario-*.json`) with semantic search via Voyage embeddings. This is the one I'm most proud of — it actually knows the regulations.
- **Runway** — financial forecasting and budget allocation for lottery programs.

**Platform features:**
- **Home Base** — internal team communication. Posts, comments, reactions, scheduled publishing, digest emails, global vs. org-scoped posts. Biggest route file in the codebase (~2,300 lines).
- **Content Calendar** — event scheduling with AI-powered content suggestions and reminders.
- **Knowledge Base** — 50+ pre-built FAQ entries, custom entries, document import (PDF/DOCX/XLSX) with chunking and semantic search.
- **Shopify Integration** — OAuth to Shopify stores, order/product/customer sync, ShopifyQL analytics, 15-minute incremental sync.
- **Rules of Play Generator** — AI-generated lottery rules documents from templates in `services/ropTemplates.js`.
- **Feed Dashboard** — external feed aggregation.
- **Voice Fingerprint** — analyzes approved responses to learn per-tool writing style. Uses Haiku for cost efficiency.
- **Agent Instructions** — staff-configurable instructions that shape AI output.
- **Conversation Memory** — persistent context across Ask Lightspeed threads.

**Admin / billing:**
- Super-admin dashboard with org management, user management, and usage analytics.
- Stripe subscriptions: 14-day free trial, monthly/annual plans, usage-limit enforcement.
...
- Shopify OAuth access tokens were plaintext in the DB. Now AES-256-GCM via `services/encryption.js`. Requires `ENCRYPTION_KEY`. Legacy plaintext rows still decrypt fine and upgrade lazily on next save.
- Migration 061 hardcoded a specific Gmail as super-admin. Replaced with the `SUPER_ADMINS` env var.
- Migrations 005, 006, 009 had TBRHSF-specific seed data inline. Extracted to `backend/data/tbrhsf-seed.json`, loaded at startup when `SEED_TBRHSF=true`. The generalized product ships clean.
...
/backend
  /src
    index.js              — app entry, migrations, 5 scheduled jobs
    /routes (18 files)
      homeBase.js           ~2,310 lines  ← biggest
      askLightspeed.js      ~1,900 lines
      admin.js              ~1,320 lines
      compliance.js         ~1,030 lines
      + 14 smaller routers
    /services (21 files)
      shopifyAnalytics.js   ~1,950 lines  ← biggest service
      claude.js             — raw fetch to Anthropic (no SDK)
      shopify.js            — Shopify Admin API + GraphQL
      systemPromptBuilder.js, draftPromptBuilder.js,
        compliancePromptBuilder.js  — prompt construction (overlap)
      voiceFingerprint.js   — per-tool writing style extraction
      encryption.js         — AES-256-GCM for Shopify tokens
      urlValidator.js       — SSRF protection
      dataRetention.js      — scheduled cleanup
      embeddingService.js   — Voyage AI integration
      + 10 more (cache, email, logger, orgOnboarding, etc.)
    /middleware
      auth.js               — JWT auth, org resolution, usage limits, rate limits
  /migrations              — 65 .sql files (auto-run on startup)
  /data                    — 12 Ontario AGCO compliance KB JSONs + tbrhsf-seed.json
  /scripts                 — compliance KB import scripts
  /config                  — database.js (pg pool)
  /__tests__               — 27 Jest suites, ~452 tests

/frontend
  index.html               — marketing site + SPA (20,931 lines)
  app.js                   — client logic (24,659 lines)
  msal-browser.min.js      — vendored MSAL bundle
  *.css / *.js             — per-feature stylesheets and modules
  /about, /pricing, /case-study, /faq, /help,
    /integrations, /security, /status  — marketing subpages
  /whats-new               — 25 feature changelog pages
  /logos                   — customer lottery program logos

/OntarioPDF-FIles          — source AGCO PDFs (12 files, yes the typo is the directory name)

render.yaml                — blueprint (informational; live config is in the Render dashboard)
```
...
- `systemPromptBuilder.js`, `draftPromptBuilder.js`, `compliancePromptBuilder.js` overlap a lot. Probably one shared builder with thin wrappers if someone wants to clean it up.
...
**Frontend:**
- MSAL is vendored as a minified bundle (`frontend/msal-browser.min.js`). Replace with an npm install + SRI.
...
- Backend URL hardcoded in `app.js`. Move to build-time config before you rebrand or stand up multi-env.
- The Google OAuth client ID in the frontend looks like a leaked secret but isn't — OAuth client IDs for browser clients are public by design. The client secret is server-side where it belongs.

**Tests:**
- ~49% of route files have a test. No cross-tenant isolation test — worth adding one, it's cheap and pays rent forever.
- CI allows up to 25 lint warnings. Should be 0.
- No `engines` field in `package.json` — Node version not pinned.

**Content / licensing:**
- `OntarioPDF-FIles/` holds AGCO Lottery Licensing Policy Manual content used to seed the compliance KB. 12 pre-processed JSON files in `backend/data/` are derived from these PDFs. Ontario OGL applies — attribution required for republication. Confirm that lands cleanly with whatever you roll into your commercial offering.
...
**Microsoft Azure AD app** — similar to Google.

**Anthropic and Voyage API keys** — easy, issue new keys under your workspace.

**Gmail SMTP** — swap credentials. Worth moving to Postmark or SendGrid for prod while you're in there.
...
3. Pick a small route file (`routes/favorites.js` or `routes/feedback.js`) and trace one request end-to-end from `frontend/app.js` to Postgres and back.
...
Files I'd avoid touching in the first week: `homeBase.js`, `askLightspeed.js`, `shopifyAnalytics.js` (complex Shopify GraphQL + ShopifyQL sync logic), the migration runner in `index.js`, the scheduled-job block at the bottom of `index.js`.

**Early wins if you're looking for them:** add Sentry, pin the Node version in `package.json`, extract the scheduled jobs to a worker, add one cross-tenant isolation test, and consider adopting the Anthropic SDK for prompt caching — the system prompts are large and caching would cut costs meaningfully.
